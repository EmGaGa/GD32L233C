...
#define FLASH_SECTOR_SIZE	(4096)//(512)		//单个扇区大小
#define FLASH_BLOCK_SIZE	(16)//(8)			//每个block有8个扇区
#define FLASH_SECTOR_COUNT	(256*16)//(2048*10)	//扇区的数量
...
)
{
	...
	case DEV_FLASH:
		while(count--)
		{
			W25QXX_Read((uint8_t*)buff, sector*FLASH_SECTOR_SIZE, FLASH_SECTOR_SIZE);
			sector++;
...
)
{
	...
	case DEV_FLASH:
		...
		case GET_SECTOR_COUNT:
			*(uint32_t *)buff=FLASH_SECTOR_COUNT;
			return RES_OK;
		break;
